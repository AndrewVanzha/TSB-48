...
// идентично /local/templates/.default/components/bitrix/news/currency-exchange/bitrix/news.list/table-on-city/result_modifier.php
...
for ($ii=0; $ii<count($arResult["OFFICES"]); $ii++) {
    if ($arResult["OFFICES"][$ii]['PROPERTY_ATT_CODE_VALUE'] == $arResult['PROPERTIES']['ATT_CODE']['VALUE']) { // сравниваю по внутреннему коду
        $arResult["OFFICES"][$ii]['INT_NAME'] = $arResult['NAME'];
        $arResult["OFFICES"][$ii]['DETAIL_PAGE_URL'] = $arResult['DETAIL_PAGE_URL'];
        $arResult["OFFICES"][$ii]['INT_CODE'] = $arResult['CODE'];
        $arResult["OFFICES"][$ii]['INT_OFFICE_CODE'] = $arResult['PROPERTIES']['ATT_CODE']['VALUE'];
        $arResult["OFFICES"][$ii]['INT_OFFICE_INFO'] = $arResult['PROPERTIES']['ATT_OFFICE'];
    }
}
