...
foreach($arResult as $key=>$arItem)
{
	...
	elseif($arItem["DEPTH_LEVEL"] == "2")
	{
        ...
        if ($arItem["PARAMS"]["item_id"] == 0) {
            $arMenuItemsIDs[$curItemLevel_1][$ii] = array();
            $arAllItems[$ii] = $arItem;
            $ii += 1;
        } else {
...
//debugg($arMenuItemsIDs);
foreach ($arMenuItemsIDs as $itemIdLevel_1=>$arLevels2)
{
	//$countItemsInRow = 18;
	$countItemsInRow = 22; // максимальное количество тем в меню
