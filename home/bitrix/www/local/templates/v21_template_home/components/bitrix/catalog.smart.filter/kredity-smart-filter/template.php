...
//$arSelect = [];
$arFilter = ["IBLOCK_ID" => "60", "ACTIVE_DATE" => "Y", "ACTIVE" => "Y"];
//$arFilter = ["IBLOCK_ID" => "60", "ACTIVE_DATE" => "Y", "ACTIVE" => "Y", "PROPERTY_MAX_SUM_SPECIAL" => "756"];
...
// начальные значения, ничего не выбрано
$emptyGET = (empty($_GET))? 100 : -1;
// если выбраны максимальные значения
$empty242 = (empty($_GET["arrFilter_242_MAX"]))? 100 : -1;
$empty243 = (empty($_GET["arrFilter_243_MAX"]))? 100 : -1;
// выбран сброс
$emptyDelGET = (empty($_GET['del_filter']))? 100 : -1;
?>
<div class="bx-filter <?=$templateData["TEMPLATE_CLASS"]?> <?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL") echo "bx-filter-horizontal"?> v21-smart-filter-block" id="sBusinessCreditForm">
    <h2 class="v21-h2 v21-smart-filter-block--header">Подберите кредит</h2>
	<!--q class="bx-filter-section"-->
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get">
			...
			<div class="row">
				...
				//not prices
				foreach($arResult["ITEMS"] as $key=>$arItem) // верстка фильтра здесь
				{
					if(
						empty($arItem["VALUES"])
						|| isset($arItem["PRICE"])
					...
					if (
						...
						continue;
					?>
					<div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-12 col-md-4<?else:?>col-lg-12<?endif?> bx-filter-parameters-box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>bx-active<?endif?>">
						<span class="bx-filter-container-modef"></span>
						<?/*?><div class="bx-filter-parameters-box-title" onclick="smartFilter.hideFilterProps(this)"><?*/?>
						<div class="bx-filter-parameters-box-title">
							<span class="bx-filter-parameters-box-hint">
                                <?
                                if($arItem["ID"] == 242) {echo 'Сумма кредита';}
                                elseif ($arItem["ID"] == 243) {echo 'Срок кредита';}
						...
						<div class="bx-filter-block" data-role="bx_filter_block">
							...
							{
								case "A"://NUMBERS_WITH_SLIDER - для рублей и месяцев
									?>
                                    <??>
									<div class="col-xs-12 bx-filter-parameters-box-container-block bx-left">
										<div class="bx-filter-input-container">
											<input
												...
												onkeyup="smartFilter.keyup(this)"
											/>
                                            <span class="bx-filter-input-value-plus"></span>
										</div>
                                        <div class="bx-filter-input-addition">
                                            <?=$arItem['DISPLAY_SIGNAL']?>
                                        </div>
									</div>
                                    <?/*?>
									...
									<div class="col-xs-12 col-xs-offset-0 bx-ui-slider-track-container">
										<div class="bx-ui-slider-track" id="drag_track_<?=$key?>">
											<?
											$precision = $arItem["DECIMALS"]? $arItem["DECIMALS"]: 0;
                                            //$step = ($arItem["VALUES"]["MAX"]["VALUE"] - $arItem["VALUES"]["MIN"]["VALUE"]) / 4;
                                            $step = ($arItem["VALUES"]["MAX"]["VALUE"] - $arItem["VALUES"]["MIN"]["VALUE"]) / 3;
                                            $value1 = number_format($arItem["VALUES"]["MIN"]["VALUE"], $precision, ".", "");
                                            if($arItem["ID"] == 242) {
                                                $value1 = $value1 / 1000 . ' тыс';
                                            }
                                            ...
                                            if($arItem["ID"] == 242) {
                                                $value4 = $value4 / 1000000 . ' млн'; // для 4х меток
                                                $value5 = $value5 / 1000000 . ' млн'; // для 5и меток
                                            }
                                            if($arItem["ID"] == 243) {
                                                //$value2 = $value2 . ' мес.';
                                                $value2 = $value2 . ' год';
											...
											<div class="bx-ui-slider-pricebar-v"  style="left: 0;right: 0;" id="colorAvailableActive_<?=$key?>"></div>
											<div class="bx-ui-slider-range" id="drag_tracker_<?=$key?>"  style="left: 0;right: 0;">
												<??><a class="bx-ui-slider-handle left"  style="left:88px;" href="javascript:void(0)" id="left_slider_<?=$key?>"></a><??>
												<a class="bx-ui-slider-handle right" style="" href="javascript:void(0)" id="right_slider_<?=$key?>"></a>
											</div>
										</div>
									</div>
									<?
                                    ...
                                    if(!isset($_GET['set_filter'])) {
                                        //debugg('! set_filter');
                                        //debugg($emptyGET);
                                        if(empty($arJsParams['curMaxPrice'])) {  // выбран сброс
                                            $arJsParams['curMaxPrice'] = $arJsParams['fltMaxPrice'] * .2;
                                        }
                                        if($emptyGET == 100) { // начальные установки
                                            $arJsParams['curMaxPrice'] = $arJsParams['fltMaxPrice'] * .2;
                                        }
                                    }
                                    //debugg($arJsParams);
									...
									<script type="text/javascript"> //
										BX.ready(function(){
											window['trackBar<?=$key?>'] = new BX.Iblock.SmartFilter(<?=CUtil::PhpToJSObject($arJsParams)?>);
                                            //console.log($('.bx-filter form input'));
                                            $('.bx-filter form input').each((ix, elem) => {
                                                //console.log($(elem).val());
                                                //console.log($(elem).data('key'));
                                                if($(elem).data('key') == <?=$key?>) {
                                                    //console.log(<?//=$key?>);
                                                    //console.log($(elem).val());
                                                    if(!$(elem).val()) {     // когда value пустой
                                                        if(<?=$key?> == 243 && <?=$empty243?> == -1)  { // в случае максимального значения
                                                            //$(elem).next('.bx-filter-input-value-plus').text('мес.');
                                                            $(elem).next('.bx-filter-input-value-plus').text('год(а)');
                                                        }
                                                    }
                                                    else {     // когда в value что-то есть
                                                        if(<?=$key?> == 243 && <?=$empty243?> == -1) {
                                                            //$(elem).next('.bx-filter-input-value-plus').text('мес.');
                                                            $(elem).next('.bx-filter-input-value-plus').text('год(а)');
                                                        }
                                                    }
                                                    if(<?=$emptyGET?> == 100) { // начальные данные
                                                        $(elem).val(<?=round($arJsParams['curMaxPrice'])?>);
                                                        if(<?=$key?> == 243) {
                                                            //$(elem).next('.bx-filter-input-value-plus').text('мес.');
                                                            $(elem).next('.bx-filter-input-value-plus').text('год(а)');
                                                        }
                                                    }
                                                    if(<?=$emptyDelGET?> == -1) {  // выбран сброс
                                                        $(elem).val(<?=round($arJsParams['curMaxPrice'])?>);
                                                        if(<?=$key?> == 243) {
                                                            //$(elem).next('.bx-filter-input-value-plus').text('мес.');
                                                            $(elem).next('.bx-filter-input-value-plus').text('год(а)');
								...
								case "B"://NUMBERS - not used
									...
									<?
									break;
								case "G"://CHECKBOXES_WITH_PICTURES - not used
									?>
									<div class="col-xs-12">
									...
									<?
									break;
								case "H"://CHECKBOXES_WITH_PICTURES_AND_LABELS - not used
									?>
									<div class="col-xs-12">
									...
									<?
									break;
								case "P"://DROPDOWN - для цели кредита
