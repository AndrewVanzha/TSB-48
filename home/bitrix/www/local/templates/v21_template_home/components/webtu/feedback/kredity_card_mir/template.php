...
<!--div class="v21-container"-->
    ...
    <div class="form-block">
        ...
        <div class="form-block--right">
            <form action="<?=$_SERVER['REQUEST_URI']?>" method="POST" class="card-application--form" id="fMirCreditCardForm">
                ...
                <div class="card-application--content">
                    ...
                    <div class="card-application--form__section">
                        <div class="grid__item-1">
                            <label class="input-group">
                                <input type="text" name="LATIN" placeholder="Имя латинским" class="input-group__field"
                                    <? if (isset($arResult['POST']['LATIN'])) { ?> value="<?=$arResult['POST']['LATIN']?>" <? } ?>
                                >
                                <span class="input-group__label"><?=GetMessage("WEBTU_FEEDBACK_3_LATIN_FIO")?></span>
                                <span class="v21-input-group__warn">Обязательное поле к заполнению</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <?
                    $creditMin = $arResult['CREDIT_LIMITS']['ATT_CCM_MIN_CREDIT_LIMIT']['VALUE'];
                    $creditMax = $arResult['CREDIT_LIMITS']['ATT_CCM_MAX_CREDIT_LIMIT']['VALUE'];
                    $arResult['POST']['CREDIT_SUMM'] = (!isset($arResult['POST']['CREDIT_SUMM']))? $creditMin : $arResult['POST']['CREDIT_SUMM'];
                    ?>
                    <div class="card-application--form__section credit_summ__section">
                        <div class="grid__item-1 credit_summ">
                            <label class="input-group">
                                <span class="input-group__label"><?=GetMessage("WEBTU_FEEDBACK_3_CREDIT_SUMM")?></span>
                                <input type="number" name="CREDIT_SUMM" placeholder="" class="input-group__field"
                                       value="<?= (isset($arResult['POST']['CREDIT_SUMM']))? $arResult['POST']['CREDIT_SUMM'] : 0; ?>"
                                    min="<?=$creditMin?>" max="<?=$creditMax?>"">
                                <span class="v21-input-group__warn">Обязательное поле к заполнению</span>
                            </label>
                            <div class="left-number"><?=number_format($creditMin, 0, '', ' ')?></div>
                            <div class="right-number"><?=number_format($creditMax, 0, '', ' ')?></div>
                            <div class="currency-word">RUB</div>
                            <div class="input-group__polzun"></div>
                        </div>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        let minS = <?=$creditMin?>;
        let maxS = <?=$creditMax?>;
        let sliderInput = $('.v21 .credit_summ .input-group__field');
        let slider = $('.v21 .input-group__polzun').slider({
            range: "min",
            min: minS,
            max: maxS,
            value: sliderInput.value,  // движение ползунка
            step: 1000,
            slide: function (event, ui) {
                sliderInput.val(ui.value);  // вывод числа в окошко
            }
        });
        $('.v21 .credit_summ .input-group__field').on("input keyup", function () {
            let qNum = parseInt(this.value, 10);
            slider.slider("value", qNum);
        });
        ...
        function changeColors(scrollTop) {
            ...
            if(scrollTop > fixLevel2) {
                $('.mir-page__background-blue').css('opacity', '0');
                $('.mir-section-schemeblock').addClass('js-initial-switch');
                $('.mir-section-schemeblock').removeClass('js-color-switch');
                $('.v21 .v21-card-application').removeClass('js-color-switch');
                $('.v21 .v21-kredity-interests').removeClass('js-color-switch');
            } else if(scrollTop > fixLevel1) {
                $('.mir-page__background-blue').css('opacity', '1');
                $('.mir-section-schemeblock').removeClass('js-initial-switch');
                $('.mir-section-schemeblock').addClass('js-color-switch');
                $('.v21 .v21-card-application').addClass('js-color-switch');
                $('.v21 .v21-kredity-interests').addClass('js-color-switch');
            } else {
                $('.mir-page__background-blue').css('opacity', '0');
                $('.mir-section-schemeblock').addClass('js-initial-switch');
                $('.mir-section-schemeblock').removeClass('js-color-switch');
...
<script>
    ...
    $('#fMirCreditCardForm').submit(function (e) {
        e.preventDefault();
        //console.log('1');
